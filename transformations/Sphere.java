...
public class Sphere {
  ...
  // Dibuja con transformación
  public void draw(float[] modelMatrix, float[] vpMatrix, float[] color) {
    float[] mvpMatrix = new float[16]; // matriz final
    Matrix.multiplyMM(mvpMatrix, 0, vpMatrix, 0, modelMatrix, 0); // MVP = VP * M
    draw(mvpMatrix, color); // usa el método base
  }

  // Método base: envía todo a la GPU
  public void draw(float[] mvpMatrix, float[] color) {
    GLES20.glUseProgram(mProgram); // activa shaders

    int positionHandle = GLES20.glGetAttribLocation(mProgram, "vPosition");
    GLES20.glEnableVertexAttribArray(positionHandle); // habilita atributo
    GLES20.glVertexAttribPointer(positionHandle, 3, GLES20.GL_FLOAT, false, vertexStride, vertexBuffer); // apunta a los datos

    int mvpMatrixHandle = GLES20.glGetUniformLocation(mProgram, "uMVPMatrix");
    GLES20.glUniformMatrix4fv(mvpMatrixHandle, 1, false, mvpMatrix, 0); // envía la matriz

    int colorHandle = GLES20.glGetUniformLocation(mProgram, "uColor");
    GLES20.glUniform4fv(colorHandle, 1, color, 0); // envía color

    GLES20.glDrawElements(GLES20.GL_TRIANGLES, indexCount, GLES20.GL_UNSIGNED_SHORT, indexBuffer); // dibuja triángulos
    GLES20.glDisableVertexAttribArray(positionHandle); // desactiva

    // Si está activado el wireframe
    if (showWireframe) {
